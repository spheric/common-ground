...
scripts/tvfy/apply.mjs     # mapping + records → issue.voting blocks in data/dataset.json
...
- Issues may optionally carry `voting` — parliamentary voting records from They Vote For You
  ...
  env or repo-root `.env`). Refresh flow: `node scripts/tvfy/fetch.mjs && node
  scripts/tvfy/apply.mjs && node scripts/validate.mjs && node scripts/build.mjs`.
